...
export class MainStore {
    ...
    @action setCurrentRouteLocation(loc) {
    ...
    tryAsyncAgain(func, args, delay, counterId, message, isUpload) {
        ...
        if(this.counter.get(counterId) < StatusEnum.MAX_RETRY) {
            ...
            if(!isUpload && (window.location.href.includes('file') || window.location.href.includes('folder'))) {
    ...
    @action getProjects(page, perPage, getAll) {
        ...
        this.transportLayer.getProjects(page, perPage)
            ...
            this.projects.forEach((p) => {
                if(userId !== null) this.getAllProjectPermissions(p.id, userId);
            });
            if(getAll) {
                this.projects.forEach((p) => {
            ...
            this.nextPage = headers !== null && !!headers['x-next-page'] ? headers['x-next-page'] : null;
            this.totalItems = headers !== null && !!headers['x-total'] ? parseInt(headers['x-total'], 10) : null;
            this.loading = false;
        }).catch(ex => this.handleErrors(ex))
    }
    ...
    @action addProject(name, desc) {
        ...
        this.transportLayer.addProject(name, desc)
            ...
            .then((json) => {
                ...
                const userId = authStore.currentUser.id !== undefined ? authStore.currentUser.id : this.currentUser.id !== undefined ? this.currentUser.id : null;
                this.projects.forEach((p) => {
                    if(userId !== null) this.getAllProjectPermissions(p.id, authStore.currentUser.id);
                });
    ...
    @action deleteProjectMember(id, userId, userName, removeSelf) {
        this.transportLayer.deleteProjectMember(id, userId)
            ...
            .then(() => {
                ...
                if(!removeSelf) {
                    this.projectMembers = this.projectMembers.filter(m => m.user.id !== userId);
                } else {
                    this.projects = this.projects.filter(p => p.id !== id);
            ...
            mainStore.transportLayer.startUpload(projId, fileName, contentType, SIZE)
                ...
                .then((json) => {
                    if(!json.error) {
                        this.loading = false;
                        let uploadObj = json;
                        if (!uploadObj.id && !uploadObj.error) throw new Error('no upload was created');
    ...
    @action hashFile(file, id) {
        if (file.blob.size <= 5000000) {
            ...
            const hashWorker = require('../util/fileHashingWorker.worker.js'); //Using require because module import causes test to fail
    ...
    @action addFileVersionSuccess(id, uploadId, json) {
        ...
        if(window.location.href.includes(id)) this.getEntity(id, Path.FILE);
    ...
    @action searchObjects(query, filter, projectPostFilter, tagPostFilter, page) {
        ...
        this.transportLayer.searchObjects(query, ...filters, page)
            ...
            .then(response => {
                ...
                this.nextPage = this.responseHeaders !== null && !!this.responseHeaders['x-next-page'] ? this.responseHeaders['x-next-page'] : null;
                this.totalItems = this.responseHeaders !== null && !!this.responseHeaders['x-total'] ? parseInt(this.responseHeaders['x-total'], 10) : null;
                this.loading = false;
            }).catch(ex =>this.handleErrors(ex))
    }
