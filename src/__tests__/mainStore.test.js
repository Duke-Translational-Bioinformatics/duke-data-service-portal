...
import { sleep, respondOK, respond, respondPromiseAll }  from "../util/testUtil";
...
describe('Main Store', () => {
    ...
    it('@action deleteTag - deletes tag from a file', () => {
        mainStore.objectTags = [fake.tag_json];
        expect(mainStore.objectTags.length).toBe(1);
    ...
    it('@action addFileVersion - adds new file version', () => {
        provenanceStore = require('../stores/provenanceStore').default;
    ...
    it('@action batchDeleteItems - should delete all selected items', () => {
    ...
    it('@action deleteProjectMember - should remove user from a project', () => {
        mainStore.projectMembers = fake.project_members_json.results;
        ...
        return sleep(1).then(() => {
            ...
            expect(mainStore.projectMembers.length).toBe(0);
            expect(mainStore.error).toBeNull();
        });
    });
