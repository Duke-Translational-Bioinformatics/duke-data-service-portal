...
class Provenance extends React.Component {
    ...
    render() {
        ...
        switch(this.state.value){
            case 0:
                relationInstructions = <span><span style={styles.relationInstructions}>You are adding a Used relation</span><br/>Click on an activity node and drag to a file node
                   to show that the activity used that file.</span>;
                break;
            case 1:
                relationInstructions = <span><span style={styles.relationInstructions}>You are adding a Was Generated By relation</span>
                    <br/>Click on a file node and drag to an activity node to show that the activity
                    generated that file.</span>;
                break;
            case 2:
                relationInstructions = <span><span style={styles.relationInstructions}>You are adding a Was Derived From relation</span>
                    <br/>Click on a file node and drag to another file node to show that the file is a derivation of another
                    file. You will then have the option to confirm which file was the derivative.</span>;
                break;
        }
        ...
        if(prjPrm !== null) {
            relationTypeSelect = prjPrm === 'viewOnly' ?
                <SelectField value={this.state.value}
                             ...
                             floatingLabelStyle={{color: '#66BB6A'}}
                    ...
                    <LeftNav width={220} openRight={true} open={this.props.toggleProvEdit}>
                        <div style={styles.provEditor}>
                            ...
                            {this.props.addEdgeMode && this.state.relationMode ?
                                <Paper style={styles.provEditor.addEdgeInstruction} zDepth={1}>
                                    {relationInstructions}
                                </Paper> : null}
                            {this.state.relationMode ?
                                <RaisedButton
                                    label="Cancel"
                                    labelStyle={{fontWeight: 200, color: '#f44336'}}
                                    style={{zIndex: 9999, margin: 10, width: '80%'}}
